...
import {
  AssignmentEnrollmentStatus,
  CourseAssignment,
  CreateAssignmentOfContentResult,
  LearningPathUserAssignmentsResponse,
} from 'src/app/resources/models/assignment';
...
export class CourseState {
  ...
  getCourses({ patchState }: StateContext<CourseStateModel>, action: CourseActions.GetCourses) {
    ...
    return this.assignmentsService.getLearningPathUserAssignments(action.learningPathId).pipe(
      map((res: LearningPathUserAssignmentsResponse) =>
        res?.courseAssignments?.map((courseAssignment: CourseAssignment) =>
          this.mapLearningPathResponseToViewObj(courseAssignment, action.learningPathId)
        )
      ),
  ...
  enrollCourse(_: any, { learningPathId, courseId, dueDate }: CourseActions.EnrollCourse) {
    return this.assignmentsService.createAssignment([courseId], dueDate).pipe(
      tap((res) => {
        const successfulResult = res.userContentIdsAdded.find((r) => r.contentId === courseId);
        if (!!successfulResult) {
          ...
          // ONLY DOING this until we fully deprecate old state structure; LP consumption still using this old way
          const courseToUpdate = this.oldLpState.snapshot?.courses?.find((c) => c.courseId === courseId);
          if (!courseToUpdate) {
            return;
          }
          ...
          const oldUpdatedCourses = this.oldLpState.snapshot?.courses?.map((course) =>
            course?.courseId === mutatedCourse.courseId ? mutatedCourse : course
          );
  ...
  enrollCourses(
    ...
    { learningPathId, courseIds, dueDate }: CourseActions.EnrollCourses
  ) {
    // patchState({ isCoursesLoading: true });
    return this.assignmentsService.createAssignment(courseIds, dueDate).pipe(
      tap((res) => {
        this.store
          .dispatch(new CourseActions.GetCourses(learningPathId))
  ...
  dropCourse({ getState, patchState }: StateContext<CourseStateModel>, { course }: CourseActions.DropCourse) {
    ...
    patchState({ isCoursesLoading: true });
    return this.assignmentsService.removeAssignments([course.enrollmentId]).pipe(
  ...
  private mapLearningPathResponseToViewObj(courseAssignment: CourseAssignment, lpId: string): CourseViewData {
    ...
    return {
      ...
      contentIconUrl: this.contentTypesService.getContentInfoIconUrl(
        courseAssignment?.type,
        courseAssignment?.documentType
      ),
      actionBtnText: this.translationService.getTranslationFileData(translationTextKey),
    };
  }
